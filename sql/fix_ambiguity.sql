-- =============================================
-- RESOLVE OVERLOADING AMBIGUITY
-- Drop all ambiguous versions of merge_daily_stats
-- then re-create the single master version.
-- =============================================

-- 1. Drop old versions (with their specific signatures)
...
-- 2. Re-create the single Master Version (using UUID for p_user_id)
...
BEGIN
    -- [The logic remains identical to previous ingestion_rpc.sql]
    ...
    ELSE
        ...
        LOOP
            IF v_updated_metrics ? 'pdf_counts' AND v_updated_metrics->'pdf_counts' ? v_pdf_key THEN
                v_updated_metrics := jsonb_set(
                    v_updated_metrics,
