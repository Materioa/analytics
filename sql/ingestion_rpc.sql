-- =============================================
-- Materio Analytics v4: ATOMIC INGESTION RPC
-- Handlers: merge_daily_stats
-- Fixes: Overwriting metrics, mismatching keys, zeroed logs
-- =============================================
...
BEGIN
    -- 1. Find existing record for this user/date
    ...
    IF v_existing_id IS NULL THEN
        -- 2. INSERT NEW RECORD
        -- Standardize keys on fresh insert
        ...
        VALUES (p_anon_id, p_user_id, p_date, v_updated_metrics, v_updated_usermeta);
    ELSE
        -- 3. ATOMIC UPDATE (SUMMING)
        
        -- A. Update Metrics (Reading Time)
        v_updated_metrics := v_existing_metrics;
        
        -- Add total reading seconds
        ...
        -- B. Update Usermeta (Engagement & Session)
        v_updated_usermeta := v_existing_usermeta;
        
        -- Add engagement seconds
        ...
        -- Deep Merge Session/State (if provided)
        ...
        -- Merge Click Trackers (summing counts)
