-- =============================================
-- NUCLEAR CLEANUP & FINAL FIX
-- This script nukes ALL versions of the function 
-- to ensure NO ambiguity remains.
-- =============================================
...
DECLARE 
    r RECORD;
BEGIN
    -- Find and drop every function named merge_daily_stats in public or auth (if any)
    FOR r IN (
        SELECT n.nspname, p.proname, pg_get_function_identity_arguments(p.oid) as args
        FROM pg_proc p
        JOIN pg_namespace n ON p.pronamespace = n.oid
        WHERE p.proname = 'merge_daily_stats'
    ) LOOP
...
-- Now recreate the definitive Master version
...
DECLARE
    ...
    v_user_uuid UUID;
BEGIN
    -- Defensive cast for p_user_id (can be UUID-string, "null", or empty)
    ...
    -- 1. Locate existing daily bucket
    ...
    ELSE
        ...
        -- FINAL UPDATE (The COALESCE here expects (UUID, UUID) as we hardened the column earlier)
        UPDATE user_daily_stats
        SET metrics = v_updated_metrics,
            usermeta = v_updated_usermeta,
            user_id = COALESCE(v_user_uuid, user_id::UUID), -- Force cast table column to UUID for check
            updated_at = NOW()
